...
void th_formatear(const TablaHash *tabla, char *destino, size_t capacidad) {
    ...
    for (int i = 0; i < tabla->capacidad && usado < capacidad; i++) {
        int escritos = snprintf(destino + usado, capacidad - usado, "[%d] -> ", i);
        if (escritos < 0) return;
        usado += (size_t)escritos;
        
        THNodo *actual = tabla->buckets[i];
        while (actual != NULL && usado < capacidad) {
            escritos = snprintf(destino + usado, capacidad - usado, "(%d:%d) -> ", actual->clave, actual->valor);
            if (escritos < 0) return;
            usado += (size_t)escritos;
            actual = actual->siguiente;
        }
        ...
        if (usado < capacidad) {
            escritos = snprintf(destino + usado, capacidad - usado, "NULL\n");
            if (escritos > 0) usado += (size_t)escritos;
        }
    }
}
